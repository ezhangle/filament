 ...
 * that a single bundle corresponds to a single texture object.
 *
 * One bundle may be comprised of a several mipmap levels, cubemap faces, and array elements. The
...
public:
    ...
    /**
     * Creates an empty (invalid) bundle.
     */
    KtxBundle() {}
    ...
    /**
     * Creates a new bundle by deserializing the given data. (consumes the contents of a KTX file)
     */
    ...
    /**
     * Makes a shallow copy with shared data.
     */
    KtxBundle& operator=(const KtxBundle& that);
    KtxBundle(const KtxBundle& that);

    /**
     * Gets or sets information about the texture object, such as format and type.
     */
    KtxInfo const& getInfo() const;
    KtxInfo& info();
     ...
     */
    bool getBlob(KtxBlobIndex index, uint8_t const** data, uint32_t* size) const;
    ...
    void reset() { *this = KtxBundle(); }
    bool isValid() const { return mImplRef; }

private:

    struct SharedReference;
    SharedReference* mImplRef = nullptr;
};
